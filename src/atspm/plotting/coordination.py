...
Gap Marker Rule:
    df_signal rows with event_code == -1 are excluded before
...
def plot_coordination(
    ...
    Args:
        ...
        df_signal: Signal-event DataFrame with columns:
            ...
            * ``event_code``  – int (1=Green, 8=Yellow, 9=RedClearance,
              11/12=Red, -1=Gap).
        ...
        df_det: Optional detector events DataFrame with columns:
            ...
            * ``timestamp``   – datetime-like event timestamp.
            * ``event_code``  – int (82=actuation on, 81=actuation off).
            * ``parameter``   – int detector number.
            * ``cycle_start`` – datetime-like cycle boundary.
            * ``t_cs``        – float seconds from ``cycle_start`` to
              ``timestamp`` (pre-computed in SQL; computed here if absent).

        det_config: Optional dict or pandas Series mapping config
    ...
    # Changed: Use native event_code column
    df_signal = df_signal[df_signal['event_code'] != _GAP_CODE].copy()

    if 'Duration' not in df_signal.columns:
        df_signal = _compute_durations(df_signal)

    # Retain only Green / Yellow / Red-Clearance codes.
    df_signal = df_signal[df_signal['event_code'].isin([1, 8, 9])].copy()
    df_signal['parameter'] = pd.to_numeric(df_signal['parameter'], errors='coerce').astype('Int64')
    df_signal = df_signal.dropna(subset=['parameter', 'Duration'])
    
    df_signal['Duration'] = df_signal['Duration'].clip(upper=300.0)
    df_signal['gyr'] = df_signal['event_code'].map(_CODE_TO_GYR)
...
def _compute_initial_zoom(
    ...
    # Changed: Use native cycle_start column
...
def _add_ring_bars(
    ...
    # Changed: parameter instead of ID
    sig = df_signal[df_signal['parameter'].isin(ring_phases)].copy()
    if sig.empty:
        return

    sig = sig.sort_values(['cycle_start', 'parameter', 'timestamp'])

    # Changed: Use native columns
    green_mask = sig['event_code'] == 1
    sig.loc[green_mask, '_green_count'] = (
        sig.loc[green_mask]
        .groupby(['cycle_start', 'parameter'])
        .cumcount()
    )
    sig['_seq_pos'] = (
        sig.groupby(['cycle_start', 'parameter'])['_green_count']
    ...
    combos = sig[['parameter', 'gyr', '_seq_pos']].drop_duplicates().copy()
...
def _add_coord_plan_markers(
    ...
    # Changed: Uses coord_plan and cycle_start
...
def _add_detector_traces(
    ...
    # Changed: Uses event_code native naming
    df_on = df_det[df_det['event_code'] == 82].copy()
    if df_on.empty:
    ...
    for key, det_str in cfg_items_sorted:
        ...
        det_segments: List[Tuple[int, Any, Any]] = []
        ...
        for det_id_str in det_str.split(','):
            ...
            # Changed: parameter instead of ID
            df_d = df_on[df_on['parameter'] == det_id].copy()
            if df_d.empty:
                continue
...
def _compute_t_cs(df_det: pd.DataFrame) -> pd.DataFrame:
    """Compute ``t_cs``: seconds from ``cycle_start`` to ``timestamp``."""
    df = df_det.copy()
    ts = df['timestamp']
    cs = df['cycle_start']
...
def _parse_det_key(key: str) -> Tuple[Optional[int], Optional[str]]:
    """Parse a config key into ``(phase_number, type_key)``."""
    key = str(key).strip()
    parts = key.split()
    if len(parts) < 2:
        return None, None
...
def _compute_t_start(df_signal: pd.DataFrame) -> pd.DataFrame:
    """Compute ``t_start``: seconds from ``cycle_start`` to ``timestamp``."""
    df = df_signal.copy()
    ts = df['timestamp']
    cs = df['cycle_start']
...
def _compute_durations(df_signal: pd.DataFrame) -> pd.DataFrame:
    ...
    # Changed: Uses parameter, cycle_start, timestamp
    df = df_signal.sort_values(['parameter', 'cycle_start', 'timestamp']).copy()

    ts = df['timestamp']
    df['_ts_end'] = df.groupby(['parameter', 'cycle_start'])['timestamp'].shift(-1)

    if pd.api.types.is_float_dtype(ts):
        df['Duration'] = (df['_ts_end'] - df['timestamp']).clip(lower=0.0)
    else:
        df['Duration'] = (
            pd.to_datetime(df['_ts_end']) - pd.to_datetime(df['timestamp'])
        ).dt.total_seconds().clip(lower=0.0)
...
def _validate_signal_columns(df: pd.DataFrame) -> None:
    ...
    # Changed: Validate against native database schema
    required = {'timestamp', 'event_code', 'parameter', 'cycle_start'}
