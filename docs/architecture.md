...
`*Engine` classes (`AogEngine`, `CountEngine`, `PhaseEngine`, `DetectorEngine`, `FlowRateEngine`, `CriticalMovementEngine`) sit at the Shell/Core boundary: each wraps a `db_path` (+ optional `timezone`), reads events via `reader.py`, and delegates the actual computation to a Core function (`arrival_on_green`, `vehicle_counts`/`ped_counts`, `phase_splits`, `analyze_discrepancies`, `flow_rate`/`rate_profiles`, `critical_movement_analysis`). They share the same shape — constructor takes `db_path`, public methods return DataFrames or write CSV via an optional `output_dir` argument, and binned results carry `coverage`/`data_quality` columns sourced from `ingestion_log`. `CriticalMovementEngine` is the one that composes: it sources its demand from `CountEngine` rather than reading events itself.
...
- **UTC epoch floats** — all timestamps are stored as `REAL` (UTC epoch seconds).
- **Intersection time, never machine time** — a *naive* datetime anywhere in the package means the **intersection's** local wall clock; an *aware* one keeps its own offset. Neither ever consults the host machine's clock, so a `US/Mountain` database returns identical rows whether it's read from Boise, London, or CI. Query bounds convert through `utils.timezone.to_epoch` — never `datetime.timestamp()`, which silently resolves a naive value through the host's zone. Naive bounds pick their zone as: explicit `timezone=` argument → the DB's `metadata.timezone` → `DEFAULT_TIMEZONE` (`US/Mountain`). See the Timezone contract in [api_reference.md](api_reference.md#timezone-contract).
- **Gap markers** — `event_code = -1` marks a data discontinuity (controller reset, missing file, backward controller clock set, etc.). Any logic computing a duration or pairing sequential events must stop at a gap marker rather than bridge across it. See [database_schema.md](database_schema.md#gap-markers).
