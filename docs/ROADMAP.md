...
3. **Phase 2 — I/O-bound, throwaway SQLite fixture via `DatabaseManager` — Sonnet #4 ✅ DONE.** `tests/conftest.py`: `empty_db` fixture builds a tmp_path DB via `DatabaseManager.init_db()` (events/config/metadata/ingestion_log, WAL) plus `CycleProcessor._init_cycles_table()` (cycles is created lazily by the app itself, not by `init_db`) — real schema code throughout, no hand-written DDL. `seed_events()` helper inserts raw rows and can plant an `event_code = -1` gap marker. Happy-path smoke tests added: `tests/data/test_manager.py` (`get_metadata`) and `tests/data/test_reader.py` (`check_data_quality`, including a seeded gap marker counted correctly). Real edge-case assertions (missing `metadata` table → `{"timezone": "US/Mountain"}` fallback, zero-row metadata table, quality-gap scoring) left as `# TODO(fable):` stubs in both new test files for a Fable edge-case pass.
